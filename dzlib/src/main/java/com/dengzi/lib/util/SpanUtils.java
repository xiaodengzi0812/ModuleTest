...
public final class SpanUtils {
    ...
    private void updateCharCharSequence() {
        ...
        if (iconMarginGapWidth != -1) {
            if (iconMarginBitmap != null) {
                ...
                mBuilder.setSpan(new CustomIconMarginSpan(DZUtil.getContext(), iconMarginUri, iconMarginGapWidth, alignIconMargin), start, end, flag);
            } else if (iconMarginResourceId != -1) {
                mBuilder.setSpan(new CustomIconMarginSpan(DZUtil.getContext(), iconMarginResourceId, iconMarginGapWidth, alignIconMargin), start, end, flag);
            }
        }
    ...
    private void updateImage() {
        ...
        int end = start + 5;
        if (imageBitmap != null) {
            mBuilder.setSpan(new CustomImageSpan(DZUtil.getContext(), imageBitmap, alignImage), start, end, flag);
        } else if (imageDrawable != null) {
            mBuilder.setSpan(new CustomImageSpan(imageDrawable, alignImage), start, end, flag);
        } else if (imageUri != null) {
            mBuilder.setSpan(new CustomImageSpan(DZUtil.getContext(), imageUri, alignImage), start, end, flag);
        } else if (imageResourceId != -1) {
            mBuilder.setSpan(new CustomImageSpan(DZUtil.getContext(), imageResourceId, alignImage), start, end, flag);
        }
    }
