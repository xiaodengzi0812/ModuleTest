...
public final class ProcessUtils {
    ...
    public static String getForegroundProcessName() {
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            PackageManager packageManager = DZUtil.getContext().getPackageManager();
            Intent intent = new Intent(Settings.ACTION_USAGE_ACCESS_SETTINGS);
            List<ResolveInfo> list = packageManager.queryIntentActivities(intent, PackageManager.MATCH_DEFAULT_ONLY);
            System.out.println(list);
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(DZUtil.getContext().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) DZUtil.getContext().getSystemService(Context.APP_OPS_SERVICE);
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        DZUtil.getContext().startActivity(intent);
                    }
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        return null;
                    }
                    UsageStatsManager usageStatsManager = (UsageStatsManager) DZUtil.getContext().getSystemService(Context.USAGE_STATS_SERVICE);
    ...
    public static Set<String> getAllBackgroundProcesses() {
        ActivityManager am = (ActivityManager) DZUtil.getContext().getSystemService(Context.ACTIVITY_SERVICE);
